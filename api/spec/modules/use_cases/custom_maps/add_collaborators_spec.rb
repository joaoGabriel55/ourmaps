...
RSpec.describe UseCases::CustomMaps::AddCollaborators do
  ...
  context "add collaborators to a custom map" do
    let(:add_custom_map_collaborators) do
      described_class.new(map: custom_map, owner_id: custom_map.owner.id, collaborators:, adapter:)
    end

    it "calls custom map repository" do
      allow(adapter).to receive(:add_collaborators!).and_return(nil)

      add_custom_map_collaborators.call

      expect(adapter).to have_received(:add_collaborators!).with(map_id: custom_map.id, collaborators:)
    end
  end

  context "add collaborators custom map raise error" do
    let(:add_custom_map_collaborators) do
      described_class.new(map: custom_map, owner_id: custom_map.owner.id, collaborators: [custom_map.owner.id], adapter:)
    end

    before { allow(adapter).to receive(:add_collaborators!).and_raise(StandardError) }
